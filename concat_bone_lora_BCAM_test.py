...
# 参数
# =========================
TEST_CSV_PATH = './data/resnet/test_bone_meta_res.csv'   # 测试集CSV路径
BEST_MODEL_PATH = './output3/bone/LORA/concat_5fold_best_adapter.pth'  # 训练保存的最佳模型
BATCH_SIZE = 32
...
plt.grid(False)
plt.tight_layout()
# plt.savefig("./output3/concat/concat_test_roc_CI", dpi=300)
plt.savefig("./output3/bone/LORA/concat_test_roc_CI", dpi=300)
plt.show()
...
pd.DataFrame({
    "label": test_labels,
    "prob": test_probs
}).to_csv("./output3/bone/LORA/test_predictions.csv", index=False)
...
plt.grid(False)
plt.tight_layout()
# plt.savefig("./output3/concat/concat_test_prc_CI", dpi=300)
plt.savefig("./output3/bone/LORA/concat_test_prc_CI", dpi=300)
plt.show()