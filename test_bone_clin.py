...
# 配置
TEST_CSV = './data/resnet/test_bone_meta_res.csv'
BEST_MODEL_PATH = './output2/bone_clin.pt'
BATCH_SIZE = 32
...
#
# # 保存预测概率和标签
# np.save('./output/bone_clinical_probs_only_zhuan.npy', df_test_sorted['prob'].values)
# np.save('./output/bone-labels_only_zhuan.npy', df_test_sorted['label'].values)
# np.save('./output/bone_exam_ids_only_zhuan.npy', df_test_sorted['exam_id'].values)
df_test_sorted[['exam_id', 'label', 'prob']].to_csv('./output2/bone_clin_test_probs.csv', index=False)
...
with open("./output2/bone_clin_roc.pkl", "wb") as f:
...
plt.grid(False)
plt.tight_layout()
plt.savefig('./output2/bone_clin_test_roc.png', dpi=300)  # 保存图片
plt.show()
...
plt.grid(False)
plt.tight_layout()
plt.savefig("./output2/bone_clin_prc.png", dpi=300)
plt.show()