...
# 配置
TEST_CSV_PATH = './data/resnet/test_bone_meta_res.csv'
BEST_MODEL_PATH = './bone_img.pt'
BATCH_SIZE = 32
...
#
#  保存预测概率
test_df_sorted[['exam_id', 'label', 'prob']].to_csv('./output2/bone_image_test_probs.csv', index=False)
print("图像预测概率已保存。")
...
with open("./output2/bone_img_roc.pkl", "wb") as f:
...
plt.grid(False)
plt.tight_layout()
plt.savefig("./output2/bone_img_roc.png", dpi=300)
plt.show()
...
plt.grid(False)
plt.tight_layout()
plt.savefig("./output2/bone_img_prc.png", dpi=300)
plt.show()