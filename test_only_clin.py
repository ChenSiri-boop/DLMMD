...
# 配置
TEST_CSV = './data/resnet/test_meta_res.csv'
...
# 保存预测概率和标签
# np.save('./output/clinical_probs_5.27.npy', df_test_sorted['prob'].values)
# np.save('./output/labels_5.27.npy', df_test_sorted['label'].values)
# np.save('./output/exam_ids_5.27.npy', df_test_sorted['exam_id'].values)
df_test_sorted[['exam_id', 'label', 'prob']].to_csv('./output2/clin_test_probs.csv', index=False)
...
with open("clin_roc.pkl", "wb") as f:
...
plt.grid(False)
plt.tight_layout()
plt.savefig('./output2/clin_test_roc.png', dpi=300)  # 保存图片
plt.show()
...
plt.grid(False)
plt.tight_layout()
plt.savefig("./output2/clin_test_prc.png", dpi=300)
plt.show()