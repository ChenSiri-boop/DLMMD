...
BEST_MODEL_PATH = './output2/bone_img.pt'
...
def save_best_fold(model, train_df, val_df, all_probs_val, best_acc):
    ...
    combined_df.to_csv('./output2/bone_img_train_val_probs.csv', index=False)
    ...
    # 保存
    train_df_processed.to_csv('./output2/bone_best_fold_train.csv', index=False)
    val_df_processed.to_csv('./output2/bone_best_fold_val.csv', index=False)
...
if __name__ == '__main__':
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig('./output2/bone_img_5fold_roc.png', dpi=300)
    plt.show()
